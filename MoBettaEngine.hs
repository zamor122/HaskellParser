...
-- Be sure to run cabal install hashmap before running - Josh
-- also try cabal install unordered-containers
...
printAction intCalc = do -- Print n to the screen
  n <- intCalc
  doIO $ putStr (show n)

-- Compute an integer, then store it
assignAction :: String -> IntCalc -> Action
assignAction v intCalc = do -- Assign v and make sure to store it
  n <- intCalc
  updateEnv v n
...
ifAction boolCalc action1 action2 = do
  checkTestCase <- boolCalc -- Check the case (condition)
  if checkTestCase then action1 else action2 -- choose the computation

whileAction :: BoolCalc -> Action -> Action
whileAction boolCalc action = do
  checkTestCase <- boolCalc
  when checkTestCase loop
  where
    loop = do
      action
...
boolCalc (BoolConst b) = return b -- return bool
boolCalc (Reln cOp expr1 expr2) = do
  n1 <- (intCalc expr1)
  n2 <- (intCalc expr2)
  let func = (opLookup cOp relnOps)
  return $ func n1 n2
boolCalc (BBin op expr1 expr2) = do
  n1 <- (boolCalc expr1)
  n2 <- (boolCalc expr2)
  let func = (opLookup op bBinaryOps)
  return $ func n1 n2
boolCalc (BUn op expr) = do
  n1 <- (boolCalc expr)
  let func = (opLookup op bUnaryOps)
  return $ func n1
...
intCalc (ABin op expr1 expr2) = do
  n1 <- (intCalc expr1)
  n2 <- (intCalc expr2)
  let func = (opLookup op aBinaryOpTable)
  return $ func n1 n2

intCalc (AUn op expr) = do
  n1 <- (intCalc expr)
  let func = (opLookup op aUnaryOps)
  return $ func n1
